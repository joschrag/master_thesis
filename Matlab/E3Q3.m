...
p_root = sturm(double(coef),50000,10^-40);
...
for i=1:size(result,1)
    n(i) = norm(vpa(subs(c*v,[x,y,z],result(i,:))));
    eval_str = join(repmat("\n%g",[1,m]),"");
    line_break = join(repmat("=",[1,10]),"");
    res_str = join(repmat("%g",[1,m]),",");
    res_str = join(["(x,y,z) = (",res_str,")"]);
    result_str = sprintf("Lösung %%i\n%s-> \nNorm: %%g%s\n%%s\n",res_str,eval_str);
    fprintf(result_str,i,result(i,:),n(i),vpa(subs(c*v,[x,y,z],result(i,:))),line_break);
end
end
