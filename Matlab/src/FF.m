classdef FF
    ...
    methods
        ...
        function ret = isequaln(self,other)
            arguments
                self FF
                other FF
            end
        ...
        function res = l_inv(self)
            arguments
                self FF
            end
            [m,n] = size(self.value);
            if m<n || rank(self) < n
                error("FF:l_inv","Left inverse does not exist for this matrix.")
            end
            L = FF([self.value,zeros(m,m-n),eye(m)],self.order).rref();
            res = FF(L(1:n,m+1:2*m),self.order);
        end
        function res = r_inv(self)
            arguments
                self FF
            end
            [m,n] = size(self.value);
            if n<m || rank(self) < m
                error("FF:r_inv","Right inverse does not exist for this matrix.")
            end
            res = l_inv(self')';
        end
    end
