...
for i = 1:numel(p_root)
    ...
    switch join(string(m),"")
        case "1"
            for eq = subs(equations,[p_var;lin_vars(1:2)],[p_root(i);-r(1).*lin_vars(2)-r(2);lin_vars(2)])'
                r_root = get_gf_root(coeffs(eq,lin_vars.value(2),"All"),prime);
            end
            q_root = FF(-r(1).*r_root-r(2),prime).value;
        case "2"
            for j=1:3
                eq = subs(equations(j),[p_var;lin_vars.value(1:2)],[p_root(i);lin_vars.value(1);-r(2)]);
