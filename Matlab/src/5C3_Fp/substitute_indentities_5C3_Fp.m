...
arguments
    ...
    lin_vars FF {mustBeSizeFF(lin_vars,[5,1])};
    prime (1,1) {mustBePrime};
end
% Compute substitution values
cube_1 = FF(P2.value(1,:),prime)*lin_vars;   %y^3
quad1 = FF(P2.value(2,:),prime)*lin_vars;   %y^2*z
quad2 = FF(P2.value(3,:),prime)*lin_vars;   %y*z^2
...
identities = [(cube_1)*FF(lin_vars.value(4),prime) - (quad1)*FF(lin_vars.value(3),prime);...      % (y^3*)z = (y^2*z)*y
    (cube_2)*FF(lin_vars.value(3),prime) - (quad2)*FF(lin_vars.value(4),prime);...                % (z^3)*y = (z^2*y)*z
    (quad2)*FF(lin_vars.value(3),prime) - (quad1)*FF(lin_vars.value(4),prime);...                % (y^2*z)*z = (z^2*y)*y
    (quad1) - (mixed)*FF(lin_vars.value(3),prime);...                     % (y^2*z) = (y*z)*y
    (mixed)*FF(lin_vars.value(4),prime) - (quad2)];                      % (y*z)*z = (z^2*y)
...
u = sym("u","real");
v = sym("v","real");
sub_final = subs(get_value(identities),lin_vars.value(1:2),[u;v]);
