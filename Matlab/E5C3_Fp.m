...
for i = 1:numel(p_root)
    ...
    switch join(string(m),"")
        case "1"
            [q_root,r_root] = rank1_1(r,prime);
        case "2"
            [q_root,r_root] = rank1_2(r,prime);
        case "3"
            [q_root,r_root] = rank1_3(r,prime);
        case "4"
            [q_root,r_root] = rank1_4(r,prime);
        case "12"
            [q_root,r_root] = rank2_12(r,prime);
        case "13"
            [q_root,r_root] = rank2_13(r,prime);
        case "14"
            [q_root,r_root] = rank2_14(r,prime);
        case "23"
            [q_root,r_root] = rank2_23(r,prime);
        case "24"
            [q_root,r_root] = rank2_24(r,prime);
        case "34"
            [q_root,r_root] = rank2_34(r,prime);
        case "123"
            [q_root,r_root] = rank3_123(r,prime);
        case "124"
            [q_root,r_root] = rank3_124(r,prime);
        case "134"
            [q_root,r_root] = rank3_134(r,prime);
        case "234"
            [q_root,r_root] = rank3_234(r,prime);
        case "1234"
            [q_root,r_root] = rank4_1234(r,prime);
        otherwise
...
if numel(result) > 0
    for i=1:size(result,1)
        ...
        res_str = join(repmat("%i",[1,3]),",");
        res_str = join(["(",res_str,")"]);
        result_str = sprintf("Lösung %%i %s-> \n%s\n%%s\n",res_str,eval_str);
        fprintf(result_str,i,result(i,:),mod(eval(subs(c*all_vars,[x,y,z],result(i,:))),prime),line_break);
    end
end
end
