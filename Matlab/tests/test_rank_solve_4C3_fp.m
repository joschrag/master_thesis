classdef test_rank_solve_4C3_fp < matlab.unittest.TestCase
        ...
        function test_r1234(testCase)
            for p =[3,5,7,11]
                ...
                for i=1:testCase.testParameter.num_runs
                    ...
                    [u,v] = rank4_4C3_1234_fp(r,p);
                    if ~isempty(u)
                        for vec = [u,v]'
                            res = M*FF([vec(1)^2;vec(1)*vec(2);vec(2)^2;vec(1);vec(2);1],p);
                            if any(res.value)
                                disp(r)
                                disp(M.value)
                                disp(vec)
                                ...
                                for t0=1:p
                                    for u0=1:p
                                        cntrl = M*FF([t0^2;t0*u0;u0^2;t0;u0;1],p);
                                        if ~any(cntrl.value)
                                            fprintf("%i,%i;%i,%i | %i\n",t0,u0,vec,p)
                                        end
                                    end
